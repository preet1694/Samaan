...
export const CarrierDashboard = () => {
  ...
  useEffect(() => {
    ...
    const storedUser = JSON.parse(localStorage.getItem("user")); // Assuming user data is stored as { email: "xyz@example.com" }
    if (storedUser) {
      setUserEmail(storedUser.email);
      fetchTrips(storedUser.email);
    }
  }, []);

  // Fetch trips where email matches the logged-in user's email
  const fetchTrips = async (email) => {
    try {
      const response = await axios.get(`http://localhost:5000/api/trips?email=${email}`);
      ...
      <div className="min-h-screen bg-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <DashboardCard icon={<MapPin />} title="Total Destinations" value={trips.length} />
            <DashboardCard icon={<Calendar />} title="Upcoming Trips" value={trips.length} />
            <DashboardCard icon={<DollarSign />} title="Total Earnings" value={`$${trips.length * 50}`} />
          </div>
